...
import { fetchSuggestedPeople } from '../store/suggestionsSlice';
import { cancelFriendRequest, sendFriendRequest } from '../store/socialSlice';
import { fetchEvents, setEventRsvp } from '../store/eventsSlice';

const LeftSidebar = () => {
    const dispatch = useDispatch();
    const suggestedPeople = useSelector((state) => state.suggestions?.people || []);
    const suggestedLoading = useSelector((state) => state.suggestions?.loading);
    const pending = useSelector((state) => state.social?.pending || {});
    const friendRequestSentTo = useSelector((state) => state.social?.friendRequestSentTo || {});
    ...
    const upcomingEvents = React.useMemo(() => {
        ...
        return events
            .filter((e) => {
                const ts = e?.starts_at ? new Date(e.starts_at).getTime() : NaN;
    ...
    const getEventDateParts = (startsAt) => {
    ...
    const handleCancelFriendRequest = async (userId, requestId) => {
        try {
            await dispatch(cancelFriendRequest({ userId, requestId })).unwrap();
        } catch (err) {
                    ...
                    <div className="_left_inner_area_suggest _padd_t24  _padd_b6 _padd_r24 _padd_l24 _b_radious6 _feed_inner_area">
                        <div className="_left_inner_area_suggest_content _mar_b24">
                            ...
                            <span className="_left_inner_area_suggest_content_txt">
                                <Link className="_left_inner_area_suggest_content_txt_link" to="/find-people">See All</Link>
                            </span>
                        </div>
                        {suggestedLoading ? (
                            <p style={{ marginBottom: 0, fontSize: '14px' }}>Loading...</p>
                        ) : suggestedPeople.length === 0 ? (
                            <p style={{ marginBottom: 10, fontSize: '14px' }}>No suggestions yet.</p>
                        ) : (
                            suggestedPeople.slice(0, 3).map((person) => {
                                const name = `${person.first_name || 'User'} ${person.last_name || ''}`.trim();
                                const subtitle = person.role || person.bio || '';
                                const avatarSrc = person.profile_photo || '/assets/images/Avatar.png';
                                const sentRequestValue = friendRequestSentTo?.[person.id];
                                const hasSentRequest = Boolean(sentRequestValue);
                                const requestId = typeof sentRequestValue === 'number' ? sentRequestValue : null;
                                const isSending = !!pending[`friendRequest:${person.id}`];
                                const isCanceling = !!pending[`cancelFriendRequest:${person.id}`];
                                                ...
                                                <Link to={`/profile/${person.id}`}>
                                                    <img 
                                                        ...
                                                        style={{ 
                                                            ...
                                                            objectFit: 'cover',  // Prevents the image from squishing
                                                            width: '45px',       // (Optional) ensures they are all the exact same size
                                                            height: '45px'       // (Optional) ensures they are perfectly square before circling
                                                        }} 
                                                    />
                                                </Link>
                                            </div>
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    if (hasSentRequest) {
                                                        ...
                                                        handleCancelFriendRequest(person.id, requestId);
                                                        return;
                                                    }
                                                    handleFriendRequest(person.id);
                                                }}
                                                disabled={isSending || isCanceling || (hasSentRequest && requestId == null)}
                                            >
                                                {isSending || isCanceling ? '...' : hasSentRequest ? 'Cancel' : 'Connect'}
                                            </button>
                                        </div>
                                    </div>
                                );
                ...
                <div className="_layout_left_sidebar_inner">
                    <div className="_left_inner_area_event _padd_t24  _padd_b6 _padd_r24 _padd_l24 _b_radious6 _feed_inner_area">
                        ...
                        {upcomingEvents.length === 0 ? (
                            <p style={{ marginBottom: 0, fontSize: '14px' }}>No upcoming events.</p>
                        ) : (
                            upcomingEvents.map((event) => {
                                const eventId = event?.id;
                                const { day, month } = getEventDateParts(event?.starts_at);
                                const isGoing = event?.my_rsvp_status === 'going';
                                const goingCount = Number(event?.going_count) || 0;
                                const isBusy = !!eventsPending?.[`rsvp:${eventId}`];

                                return (
                                    <div className="_left_inner_event_card" key={eventId}>
                                        <Link className="_left_inner_event_card_link" to="/events">
                                            ...
                                            <div className="_left_inner_event_card_content">
                                                ...
                                                <div className="_left_inner_card_txt">
                                                    <h4 className="_left_inner_event_card_title">{event?.name || 'Untitled event'}</h4>
                                                </div>
                                            </div>
                                        ...
                                        <div className="_left_inner_event_bottom">
                                            ...
                                            <a
                                                ...
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    if (isBusy) return;
                                                    dispatch(setEventRsvp({ eventId, status: isGoing ? null : 'going' }));
                                                }}
